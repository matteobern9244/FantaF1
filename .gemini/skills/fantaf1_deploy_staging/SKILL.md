---
name: fantaf1_deploy_staging
description:
  Use this skill to execute the FantaF1 23-point staging deployment protocol
  whenever the 'deploya-staging' command is invoked, using a coordinated
  sub-agent workflow.
---

# Instructions: FantaF1 23-Point Staging Deployment Protocol (develop -> staging)

You are the **Staging Release Orchestrator**, responsible for coordinating the
full FantaF1 23-point deployment protocol for the staging environment. You MUST
delegate each phase to the specialized **generalist** sub-agent as described
below.

## Core Mandates

- **Source Branch:** The process MUST start from the `develop` branch.
- **Target Branch:** The target for the release candidate is the `staging`
  branch.
- **Authoritative Source:** The 23 points are defined verbatim in `AGENTS.md`
  (Section 10), adapted here for the staging target.
...
## The 23-Point Protocol (Adapted for Staging)

1.  Before starting, run a full preflight on the repository state and release
    target. Verify there are no unstaged files, `staging` is aligned with the
    stack intended for certification, the current branch is exactly `develop`,
    the branch is synced with its remote, required `git` and `gh` authentication
    are available, the required runtime/toolchain versions are present, the
    minimum required environment variables and deploy secrets exist, and the
    staging release target is still valid. If any of these checks fail, stop
...
2.  Run a dry-run summary before any mutating action. Show the computed next
    ...
    Pull Request target (`staging`), and any tag/release names that would be
    created (if applicable for staging). Do not commit, push, tag, or release
    during the dry-run phase.
3.  Determine the correct next application version (e.g., a prerelease or the
    next patch/minor) and bump it consistently across the repository wherever
    needed.
...
9.  If any test, lint, build, or mandatory validation fails, stop immediately.
    ...
    checks, and proceed only when the repository is deployable to staging again.
10. Create an intelligent commit message that accurately summarizes the work
    performed for the staging release.
11. Commit all required changes.
12. Push the current working branch (`develop`) to its remote branch.
13. Create or update a Pull Request from `develop` into `staging`.
...
15. Enable auto-merge on that Pull Request using the repository's configured
    merge method, without bypassing branch protection on `staging`.
16. Wait until the Pull Request is either merged by GitHub after all required
    ...
    stayed pending.
17. If the Pull Request is not merged successfully into `staging`, stop
    immediately, do not create tags or releases, and report the exact blocking
    state.
18. After GitHub has merged the Pull Request into `staging`, verify that the
    merged commit on `staging` is the expected release commit and that the
    repository state still matches the validated staging candidate.
19. (Optional for Staging) Create a tag on `staging` if the project uses staging
    tags.
20. Verify that the created tag points to the correct merged commit before
    proceeding.
21. (Optional for Staging) Create a GitHub Release marked as "Pre-release" if
    desired.
22. If tag creation, release creation, or any post-merge release step fails,
    stop immediately and report the exact rollback or cleanup actions required.
23. Return to the original branch from which the staging deployment workflow
    started.

## The Coordinated Staging Deployment Workflow

### Phase A: Staging Release Planner (Points 1-6)

- **Sub-agent:** `generalist` (Staging Release Planner).
- **Task:**
...
### Phase B: Staging Quality Gatekeeper (Points 7-9)

- **Sub-agent:** `generalist` (Staging Quality Gatekeeper).
- **Task:** Execute full validation pipeline including targeted checks for
  high-risk areas or workflows (Points 7-9).

### Phase C: Staging Integration Manager (Points 10-17)

- **Sub-agent:** `generalist` (Staging Integration Manager).
- **Task:** Perform commit, push, PR creation (`develop` -> `staging`),
  configuration verification, and auto-merge monitoring (Points 10-17).

### Phase D: Staging Release Publisher (Points 18-23)

- **Sub-agent:** `generalist` (Staging Release Publisher).
- **Task:** Verify merge on `staging`, cleanup, and restore branch (Points
  18-23).
