...
Questa track esegue il dump di sicurezza del database reale. L'integrità del
backup verrà verificata post-esecuzione.

## Fase 1: Preflight e Sicurezza Workspace

**Obiettivo:** Preparare l'ambiente locale per ospitare i dati senza rischi di
commit accidentali.
...
- [ ] Task: Esecuzione mongodump (GREEN)
  - [ ] Lanciare il comando `mongodump` con l'URI fornita e output verso una
        sottocartella datata.
...
- [ ] Task: Analisi Contenuto
  - [ ] Elencare i file generati per confermare la presenza di tutte le
        collection (appdatas, weekends, etc.).
