...
- **Documentazione:** Aggiornare `AGENTS.md` e `README.md` con i nuovi numeri di
  baseline.
- **TDD:** Seguire rigorosamente il ciclo RED -> GREEN -> REFACTOR per le nuove
  aggiunte di test.
- **Non-Regressione:** Garantire che i test esistenti rimangano passanti e che
  non vengano introdotte regressioni funzionali.

## Analisi Iniziale (Gap Identificati)

1. **Node Backend (`app.js`, `server.js`):** Attualmente al 99.88% a causa dei
   blocchi `if (!process.env.VITEST)` che caricano `dotenv`.
2. **C# Backend:** Verificare lo stato attuale tramite
   `npm run test:csharp-coverage` e colmare eventuali gap.
