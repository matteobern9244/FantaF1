...
- [ ] Task: Scan `conductor/tracks/` for `metadata.json` files and identify
      tracks with `"status": "completed"`.

## Phase 2: Execution (Archiving)

- [ ] Task: Move identified completed track folders to `conductor/archive/`.
- [ ] Task: Verify that the completed tracks have been successfully moved and
      are no longer in `conductor/tracks/`.
