...
## Overview
This track aims to verify that the root `Dockerfile` is fully functional and ready for Render staging deployment. We will build the image locally and run a container simulating the Staging environment to ensure no path resolution or startup errors occur.
...
- **Portability:** Ensure the image works as expected in a containerized environment.

## Acceptance Criteria
- `docker build` completes without errors.
- `docker run` starts the application in `Staging` mode.
- `curl http://localhost:3001/api/health` returns a 200 OK with `environment: staging`.
