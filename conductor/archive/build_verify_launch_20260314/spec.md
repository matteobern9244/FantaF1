...
- **Build**: Compilazione di frontend (Vite) e backend (Express/C# quando
  applicabile).
- **Verifica**: Esecuzione dei controlli di qualità definiti in `AGENTS.md`
  (`lint`, `test`, `build`).
- **Lancio**: Avvio monitorato dell'applicazione.
- **Rischio**: Nessuna modifica al codice sorgente è prevista, quindi il rischio
  di regressione è minimo se lo stato attuale è sano.
...
4. `./start_fantaf1.command` deve avviare l'applicazione con successo e
   raggiungere lo stato di "app running".
