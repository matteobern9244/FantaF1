...
- **RED**: `npm run test:save-local` fallisce (poiché il file `.env` non esiste
  ancora).
- **GREEN**: Creazione del file `.env` con i valori corretti e verifica tramite
  lo smoke test.
...
- Identificare l'URI locale di MongoDB (es.
  `mongodb://localhost:27017/fantaf1_dev`).

### Fase 2: Creazione del file `.env`

- Creare il file `.env` nella root del progetto con i seguenti valori:
  - `MONGODB_URI=mongodb://localhost:27017/fantaf1_dev` (o quello fornito
    dall'utente se diverso).
  - `PORT=3001`
...
- Eseguire `npm run test:save-local` per confermare che il backend sia ora
  raggiungibile.
- Eseguire `npm run test:coverage` per assicurarsi che tutto il sistema sia
  stabile.
