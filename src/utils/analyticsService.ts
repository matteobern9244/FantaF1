...
type UserAnalyticsBuilderOptions = {
  mathService?: AnalyticsMathService;
};

type SeasonAnalyticsBuilderOptions = {
  mathService?: AnalyticsMathService;
  userAnalyticsBuilder?: UserAnalyticsBuilder;
};

class AnalyticsMathService {
  roundTo(value: number, digits = 1) {
  ...
  calculateConsistencyIndex(values: number[]) {
    if (values.length <= 1) {
      return 100;
    }
...
class UserAnalyticsBuilder {
  private readonly mathService: AnalyticsMathService;

  constructor({ mathService = new AnalyticsMathService() }: UserAnalyticsBuilderOptions = {}) {
    this.mathService = mathService;
  }

  buildRacePositions(history: RaceRecord[], users: UserData[]) {
  ...
  buildUserKpiSummaries(users: UserData[], history: RaceRecord[]): UserKpiSummary[] {
    const racePositions = this.buildRacePositions(history, users);
    ...
    return users.map((user) => {
      ...
      return {
        ...
        averagePosition: raceCount > 0 ? this.mathService.roundTo(totalPositions / raceCount, 1) : null,
        poleAccuracy: raceCount > 0 ? Math.round((poleHits / raceCount) * 100) : 0,
        averagePointsPerRace: raceCount > 0 ? this.mathService.roundTo(totalHistoricalPoints / raceCount, 2) : 0,
        racesCount: raceCount,
        weekendWins,
        podiums,
        averageLeaderDelta: raceCount > 0 ? this.mathService.roundTo(totalLeaderDelta / raceCount, 2) : 0,
        totalHitRate: totalPossibleHits > 0 ? Math.round((totalHits / totalPossibleHits) * 100) : 0,
      };
    });
  }

  buildFieldAccuracy(userName: string, history: RaceRecord[]): UserFieldAccuracy[] {
  ...
  buildTrend(userName: string, history: RaceRecord[]): UserGpTrendPoint[] {
    return [...history]
      .reverse()
      .map((record) => ({
  ...
  buildMostPickedDriverId(userName: string, history: RaceRecord[]) {
  ...
  buildCumulativeTrend(trend: UserGpTrendPoint[]) {
    let runningTotal = 0;
  ...
  buildPointsByField(userName: string, history: RaceRecord[]) {
    const pointsByField = {
      first: 0,
      second: 0,
      third: 0,
  ...
  buildUserAnalytics(history: RaceRecord[], userName: string): UserAnalyticsSummary {
    const trend = this.buildTrend(userName, history);
    const cumulativeTrend = this.buildCumulativeTrend(trend);
    ...
    return {
      ...
      mostPickedDriverId: this.buildMostPickedDriverId(userName, history),
      fieldAccuracy: this.buildFieldAccuracy(userName, history),
      trend,
      cumulativeTrend,
      pointsByField: this.buildPointsByField(userName, history),
...
class SeasonAnalyticsBuilder {
  private readonly mathService: AnalyticsMathService;
  private readonly userAnalyticsBuilder: UserAnalyticsBuilder;

  constructor({
    mathService = new AnalyticsMathService(),
    userAnalyticsBuilder = new UserAnalyticsBuilder({ mathService }),
  }: SeasonAnalyticsBuilderOptions = {}) {
    this.mathService = mathService;
    this.userAnalyticsBuilder = userAnalyticsBuilder;
  }

  buildSeasonComparison(users: UserData[], history: RaceRecord[], calendar: RaceWeekend[]): SeasonComparisonPoint[] {
    const weekendByMeetingKey = new Map(calendar.map((weekend) => [weekend.meetingKey, weekend]));
    const summaries = this.userAnalyticsBuilder.buildUserKpiSummaries(users, history);
    ...
    return summaries
      .map((summary) => {
        ...
        return {
          ...
          sprintPoints,
          standardPoints,
          consistencyIndex: this.mathService.calculateConsistencyIndex(userRacePoints),
  ...
  buildSeasonNarratives(comparison: SeasonComparisonPoint[]): SeasonNarrative[] {
  ...
  buildSeasonAnalytics(users: UserData[], history: RaceRecord[], calendar: RaceWeekend[]): SeasonAnalyticsSummary {
    const comparison = this.buildSeasonComparison(users, history, calendar);
